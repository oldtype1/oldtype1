...
/*	@Resource(name="uploadPath")
   ...
   public MemberVO login(MemberVO vo){
     ...
     if(vo!=null){
       ...
       * @param viewPath : 서버에서 사진을 찾기 위한 상대경로
       * @param uploadPath : 서버에 직접 사진을 저장하기 위한 절대경로
      ...
      @Override
      public void memberRegister(MemberVO mvo, ProfilePicVO pvo, String viewPath, String uploadPath)
      throws Exception {
           ...
           * 		
    				...
    				: 담겨 있는 파일의 원래 이름을 반환한다.
    				- long getSize()
    				: 파일의 크기를 볼 수 있다.
    				- boolean isEmpty()
    				: 파일 내용이 비어있으면 true, 있으면 false 반환
    				-  void transferTo(File dest)
    				: 지정된 대상 파일에 수신 된 파일을 전송.
           */
          ...
          if(file.getOriginalFilename().equals("")){ // 만약 파일의 원래 이름이 비어있다면 == if(file.isEmpty())
        	  pvo.setFilePath("http://pingendo.github.io/pingendo-bootstrap/assets/user_placeholder.png");
             	 ...
             	 file.transferTo(new File(uploadPath+fileName));	 // 설정된 path로 사진을 전송한다
